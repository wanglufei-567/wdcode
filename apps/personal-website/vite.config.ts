...
function isInsideNotesRoot(candidatePath: string): boolean {
 ...
 */
function notesContentPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/note-content', async (request, response) => {
        try {
          ...
          const targetPath = path.resolve(notesRoot, relativePath)

          if (!isInsideNotesRoot(targetPath) || isHiddenNotePath(path.relative(notesRoot, targetPath))) {
...
export default defineConfig({
  plugins: [react(), notesContentPlugin()],
  test: {
    environment: 'node',
    include: ['src/**/*.test.ts'],
  },
