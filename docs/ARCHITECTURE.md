...
外部体验的加载策略由对应项目适配器拥有：`SQLEditor` 在案例详情中直接嵌入并默认加载 **iframe**，`Flowlyte` 使用压缩后的真实截图并把下载指向外部 **GitHub Release**，`受控内容生产 Agent` 使用匿名化网站封面、架构图投影和验证快照，不加载外部运行时或暴露原始业务材料；工程实践目录不加载外部运行时，使外部资源成本只发生在进入对应案例或主动点击入口之后
